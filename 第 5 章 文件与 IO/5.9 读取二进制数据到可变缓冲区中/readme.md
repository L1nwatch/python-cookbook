...
with open('somefile', 'rb') as f:
    while True:
        n = f.readinto(buf)	# 每次都读取数据到同一个缓冲区中，不用重新申请内存之类的
        if n < record_size:
            break
...
最后，留心观察其他函数库和模块中与 `into` 相关的函数(比如 `recv_into()` ， `pack_into()` 等)。 Python的很多其他部分已经能支持直接的I/O或数据访问操作，这些操作可被用来填充或修改数组和缓冲区内容。
